...
% load real global inversion 
global_baseline = load('case5_global_ems')
  t_baseline = find(global_baseline.time == sYear);
ems_baseline = ems;
obs_baseline = global_baseline.out_anal;
ems_baseline(:,1) = global_baseline.ems_anal(t_baseline,1);
ems_baseline(:,2) = global_baseline.ems_anal(t_baseline,2);
ems_baseline(:,3) = global_baseline.ems_anal(t_baseline,3);
ems_baseline(:,4) = global_baseline.ems_anal(t_baseline,4);


% set IC to the first year of observed values
ch4c13_nh = obs.nh_ch4(1)*(1+obs.nh_ch4c13(1)/1e3);
ch4c13_sh = obs.sh_ch4(1)*(1+obs.sh_ch4c13(1)/1e3);

% [OH] comes from MCF in case5.mat

IC(1:14) = [obs.nh_ch4(1), obs.sh_ch4(1), ch4c13_nh, ch4c13_sh, obs.nh_mcf(1), obs.sh_mcf(1), obs.nh_n2o(1), obs.sh_n2o(1), IC(9), IC(10), (obs_baseline.nh_oh(t_baseline) / params.n_air)*1d9, (obs_baseline.sh_oh(t_baseline) / params.n_air)*1d9, obs.nh_co(1), obs.sh_co(1)];
%%% set isotope signatures of each sector 
signature = struct();
signature.energy = -42.95;
signature.rice = -63.2;
signature.animal = -67.1;
signature.waste = -55.4;


%%% Define new emissions 
ems_edgar = getEdgarEms('EDGAR_V7_Hemispheric.npy', sYear, eYear);
edgar_baseline = getEdgarEms('EDGAR_V7_Hemispheric.npy', sYear-1, sYear-1);;
...
result = struct();
  ...
  model_baseline = boxModel_wrapper(St,ems_baseline,IC,params);
obs_baseline = global_baseline.out_anal;
...
ems_pert(:,1) = global_baseline.ems_anal(t_baseline, 1);% - edgar_baseline.(nh_sector);

ems_pert(:,2) = global_baseline.ems_anal(t_baseline, 2);% - edgar_baseline.(sh_sector);

% add the emissions from the sector 
ems_pert(:,1) = ems_pert(:,1) + (ems_edgar.(nh_sector) - ems_edgar.(nh_sector)(1));
ems_pert(:,2) = ems_pert(:,2) + (ems_edgar.(sh_sector) - ems_edgar.(sh_sector)(1));    
...
disp(sector)
result.(sector).model.nh_ch4c13(end) - result.(sector).model.nh_ch4c13(1)
    legendLabel = sprintf(sector);

    % plot results
%    subplot(5,1,i);
y = result.(sector).model.nh_ch4c13 - model_baseline.nh_ch4c13;
result.(sector).delta.nh_ch4c13 = y;

plot(t, y, 'linewidth', 1.5, 'DisplayName', legendLabel)
      hold on
      
%    plot(t, result_struct.(sector).sh_ch4 - model_baseline.sh_ch4, 'g', 'linewidth', 1.5)
      hold on
      ylabel('per mil')
%      title(char(sector))
      ...
      xlabel('year')
plot(t, obs.nh_ch4c13 - model_baseline.nh_ch4c13, 'ko', 'linewidth', 1.5, 'DisplayName', 'observations')
hold on
%%% plot sink effects:
% OH fractionation effects
ems_pert = ems_baseline;
ems_pert(:,11) = global_baseline.ems_anal(t_baseline:end-1, 11);
ems_pert(:,12) = global_baseline.ems_anal(t_baseline:end-1, 12);
interactive_OH = true;
out_oh_sink = boxModel_wrapper(St,ems_pert,IC,params);
result.oh_sink.model = out_oh_sink;
result.oh_sink.delta.nh_ch4c13 = out_oh_sink.nh_ch4c13 - model_baseline.nh_ch4c13,;
plot(t,result.oh_sink.delta.nh_ch4c13,  'linewidth', 1.5, 'DisplayName', 'OH fractionation');
hold on
legend;
    saveas(figure(1), 'isotope_sector_noStrat_baseline.pdf', 'pdf')

%%% let's make an area plot
nh_total = zeros(length(t),1);
sh_total = zeros(length(t),1);
num_fields = length(fieldnames(result));
sectors = fieldnames(result);

for i=1:num_fields
sector = sectors{i}
nh_total = nh_total + result.(sector).delta.nh_ch4c13;
%sh_total = sh_total + result.(sector).delta.sh_ch4c13;
end
figure(2)
for i=1:num_fields
sector = sectors{i}
area(t,  result.(sector).delta.nh_ch4c13 ./ nh_total, 'DisplayName', char(sector));
hold on
end
xlabel('year')
ylabel('ppb')
legend
saveas(figure(2), 'isotope_area_plot.pdf', 'pdf')
