...
data_filename  = 'model_outputs/inversion_nostrat';
...
global use_MOPIT_CO fit_strat_exchange
...
%r = obs.nh_ch4(end-1) / obs.sh_ch4(end-1);
%obs.sh_ch4(end) = 2*1911.83 / (1+r);
%obs.nh_ch4(end) = obs.sh_ch4(end)*r;
...
out_no_strat = out;
ems_no_strat = ems_anal;
ch4_no_strat = ems_anal(:,1) + ems_anal(:,2);
...
time = [sYear:eYear];
save('case5_global_ems', 'ems_anal', 'out_anal', 'time')
