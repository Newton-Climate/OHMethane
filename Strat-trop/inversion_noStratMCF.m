...
data_filename  = 'model_outputs/inversion_noStratMCF';

%%% Describing experiment to be exported to .mat file 
fprintf('Ignoring Strat MCF Exchange and Using strat H2O to proxy Strat-trop exchange time')
...
global fit_strat_exchange ignoreStratMCFExchange
% Plotting flags
...
fit_strat_exchange = false; % are we fitting for strat exchange time?
use_strat_H2O = true; % Fix strat-trop exchange to strat H2O variability 
...
% difference in OH
kX_NH = 1.15*ones(nT,1); % s^-1 for 6600 tg/yr OH source
...
if use_strat_H2O
    stratH2O_file = fullfile(dataDir, 'obs/h2o_strat/global_strat_obs_monthly.nc');
    H2O_strat = read_strat_obs(stratH2O_file, St, tRes);

    % obtain factor to get strat-trop exchange time 
    % 
    % multiply   by -1 because higher H2O means less exchange (anti-correlated)
    ste_factor = -H2O_strat.anomaly ./ mean(H2O_strat.obs, 'omitnan') + 1; 

    ems(:,15) = 23.33*ones(nT,1); % strat-trop exchange time in years 
    ems(:,15) = ste_factor .* ems(:, 15); % fix STE to H2O strat variability 
end
...
IC(1:2) = [obs.nh_ch4(1), obs.sh_ch4(2)];
...
time = [sYear:eYear];
IC = IC_anal
save('strat_inversion', 'ems_strat', 'out_strat', 'time', 'IC_anal', 'obs')
...
time = [sYear:eYear];
IC = IC_anal
save('global_ems_strat', 'ems_anal', 'out_anal', 'time', 'IC', 'ems_anal', 'obs')
if export_data
...
r_nh = params.n_air*out_anal.nh_ch4_strat .* params.k_ch4_strat_nh ./ (params.k_12ch4*1d9*out_anal.nh_oh .* out_anal.nh_ch4);


r_sh = params.n_air*out_anal.sh_ch4_strat .* params.k_ch4_strat_sh ./ (params.k_12ch4*1d9*out_anal.sh_oh .* out_anal.sh_ch4);
